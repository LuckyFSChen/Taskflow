...
/**
 * 群組 header 的狀態優先序：需要你處理 > 失敗 > 執行中 > 排隊中 > 已完成。
 * 例：6 個任務中 4 個 completed、1 個 running、1 個等待核准 → 顯示「需要你處理」。
 */
export const GROUP_STATE_PRIORITY = ['attention', 'failed', 'running', 'queued', 'completed'];
 ...
 *
 * @param {any[]} [tasks]
 * @returns {{total:number,completed:number,running:number,queued:number,attention:number,failed:number,other:number,state:string,stateLabel:string}}
 */
export function groupSummary(tasks) {
  const counts = { completed: 0, failed: 0, attention: 0, running: 0, queued: 0, other: 0 };
