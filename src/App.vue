...
async function completionClose(options:{forceCleanup:boolean}){
  ...
  const taskId=selected.value.id;
  await run(async()=>{
    const result=await api(`/tasks/${taskId}/close`,{forceCleanup:options.forceCleanup});
    notify(result.cleanupWarning?`任務尚未關閉：${result.cleanupWarning}`:'任務已關閉。');
  });
  await loadReview();
}
