...
export function createProjectPreview({npm=runNpm,registryPath=resolve('data/preview/registry.json'),portManager=createRuntimePortManager(),onRuntimeEvent=()=>{},healthTimeoutMs=120000}={}) {
  ...
  async function startFullstack(key,path,pkg) {
    ...
    for(let attempt=1;attempt<=MAX_PORT_BIND_ATTEMPTS;attempt++){
      ...
      // 這個 lease 在 spawn 成功、bindPid() 之前都還沒有 PID 撐著；
      // 下面任何一步失敗都要放回 pool。
      let bound=false;

      try{
        const previewDbPath=resolve(
          'data/preview',
          safeKeyFragment(key),
          'taskflow.sqlite'
        );

        // 驗收身份由 AcceptanceContext 產生，
        // Preview 與 Deployment Validator 共用同一份。
        const acceptance=createAcceptanceContext({
          projectPath:path,
          key
        });
        ...
        try{
          ...
          try{
            seedStore.upsertUser(
              'TaskFlow Preview',
              acceptance.username,
              acceptance.password,
              'admin'
            );
            acceptance.injection.database=true;
          }finally{
            seedStore.close();
          }
        }catch(error){
          acceptance.injection.error=String(
            error?.message||error
          ).slice(0,200);
        }

        // 必須沿用 HEAD 的隔離邏輯：
        // Preview 不得繼承正式 TaskFlow 的 port 身分與秘密。
        const env=childEnvironment(process.env);

        env.PORT=String(port);
        env.HOST='127.0.0.1';
        env.TASKFLOW_DB_FILE=previewDbPath;

        // 如果 Preview 專案本身就是 TaskFlow，
        // 必須明確指定這一輪租到的 runtime port，
        // 絕不能繼承正式服務的 4310。
        env.TASKFLOW_PORT=String(port);
        env.TASKFLOW_HOST='127.0.0.1';

        env.PREVIEW_PORT=String(port);
        env.PREVIEW_URL=`http://127.0.0.1:${port}`;
          ...
          child.once('exit',code=>{
            if(!startupSettled){
              ...
              reject(
                new HttpError(
                  422,
                  `Preview 伺服器提前結束（code ${code}）：${stderr||'(無 stderr 輸出)'}`
                )
              );
            }
          });
        ...
        try{
          await Promise.race([
            exitPromise,
            waitForHealth(url,20000)
          ]);
        }catch(err){
          // failed startup 必須先確定 child 結束，
          // 才能釋放 lease。
          ...
          // PID 與 port 都真正釋放後才能 release。
          if(
            await waitForExit(child.pid,{timeoutMs:5000}) &&
            await waitForPortRelease(port)
          ){
            portManager.release(port);
          }

          // acquire() 與真正 bind 之間仍可能存在 race。
          if(
            attempt<MAX_PORT_BIND_ATTEMPTS &&
            isPortBindCollision(stderr)
          ){
            continue attempts;
          }
        ...
        child.once('exit',()=>{
          if(running.get(key)?.child===child){
            running.delete(key);
          }
