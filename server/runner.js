...
import { commandPermissionArgs } from './command-permissions.js';
...
export function cliAdapter({engine,prompt,cwd,schema,readOnly,runDir,onEvent,onProcess,spawnProcess=spawn,preflight=false}) {
  return new Promise((resolveResult,reject)=>{
    ...
    const args=engine==='codex'?['exec','--json','--skip-git-repo-check','--sandbox',readOnly?'read-only':'workspace-write','--output-schema',schemaPath,'-o',outputPath,'-C',cwd,'-']:['-p','--output-format','stream-json','--verbose','--json-schema',JSON.stringify(schema),'--permission-mode',readOnly?'plan':'acceptEdits','--strict-mcp-config','--mcp-config','{"mcpServers":{}}','--setting-sources','', '--tools',preflight?'Bash':readOnly?'Read,Glob,Grep,WebSearch,WebFetch':'Read,Write,Edit,Glob,Grep,Bash,WebSearch,WebFetch'];
    args.push(...(preflight&&engine==='claude'?['--allowedTools','Bash(node .taskflow/probe-child-process.cjs)','Bash(npm --version)','Bash(npm config get registry)','Bash(npm ping *)','Bash(pnpm --version)','Bash(yarn --version)']:commandPermissionArgs(engine,readOnly)));
    ...
    function line(text){try{const e=JSON.parse(text);if(e.thread_id||e.session_id)sessionId=e.thread_id||e.session_id;
      ...
      if(e.type==='assistant'){const c=e.message?.content||[];for(const block of c){if(block.type==='text'){lastSummary=block.text;onEvent(block.text);}else if(block.type==='tool_use'){if(block.name==='StructuredOutput')structuredInput=block.input;onEvent(`使用工具：${block.name}`);}}}
...
export function createRunner(store,{adapter=cliAdapter,dataDir=resolve('data'),recover=true,clock=Date.now}={}) {
  ...
  async function runTask(t,slot){let thread;const controlVersion=t.controlVersion||0;
    try {
      ...
      const adapterOptions={engine:eng,prompt,cwd:t.workspace,schema:['plan','repair_plan'].includes(phase)?planJson:resultJson,readOnly:['plan','repair_plan'].includes(phase),runDir:join(dataDir,'runs',thread.id),onEvent:message=>store.event(t.id,'activity',message,thread.id),onProcess:p=>{slot.child=p;}};
      if(['execute','repair'].includes(phase)&&needsPreflight(t)){
        ...
        if(t.dependencyPreflight?.key!==key){
          ...
          const report=await dependencyPreflight(adapter,{...adapterOptions,runDir:join(adapterOptions.runDir,'preflight')});
